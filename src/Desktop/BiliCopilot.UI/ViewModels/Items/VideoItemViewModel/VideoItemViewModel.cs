...
public sealed partial class VideoItemViewModel : ViewModelBase<VideoInformation>
{
    ...
    private void Play()
    {
        ...
        var snapshot = new MediaSnapshot(Data);
        var progress = Data.GetExtensionIfNotNull<int?>(VideoExtensionDataId.Progress);
        if (progress is not null)
        {
            snapshot.StartPosition = progress.Value;
        }
    ...
    private void PlayInPrivate()
    {
        ...
        var snapshot = new MediaSnapshot(Data, true);
        var progress = Data.GetExtensionIfNotNull<int?>(VideoExtensionDataId.Progress);
        if (progress is not null)
        {
            snapshot.StartPosition = progress.Value;
